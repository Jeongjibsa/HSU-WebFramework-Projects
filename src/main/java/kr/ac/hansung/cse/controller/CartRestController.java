...
public class CartRestController {
	...
	@RequestMapping(value = "/cartItem/plus-qty/{productId}", method = RequestMethod.PUT)
	...
	@RequestMapping(value = "/cartItem/minus-qty/{productId}", method = RequestMethod.PUT)
	public ResponseEntity<Void> minusItem(@PathVariable(value = "productId") int productId) {
		...
		// check if cartitem for a given product already exists
		for (int i = 0; i < cartItems.size(); i++) {
			if (product.getId() == cartItems.get(i).getProduct().getId() && cartItems.get(i).getQuantity() > 0) {
				CartItem cartItem = cartItems.get(i);
				cartItem.setQuantity(cartItem.getQuantity() - 1);
				cartItem.setTotalPrice(product.getPrice() * cartItem.getQuantity());
